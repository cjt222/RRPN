...
class DatasetPath(object):
    ...
    def get_file_list(self):
        if self.mode == 'train':
            return os.path.join(self.data_dir, 'ch4_training_localization_transcription_gt')
        elif self.mode == 'val':
...
def get_image_blob(roidb, mode):
    ...
    if mode == 'train' or mode == 'val':
        ...
        if mode == 'train':
            ...
            img, need_gt_boxes, need_gt_label = _rotation(img, need_gt_boxes, gt_label, prob=1.0, gt_margin=1.4)
        else:
...
def _rotation(image, gt_boxes, gt_label, prob, fixed_angle=-1, r_range=(360, 0), gt_margin=1.4):
    ...
    if np.random.rand() <= prob:
        angle = np.array(np.random.rand(1) * rotate_range - shift, dtype=np.int16)
