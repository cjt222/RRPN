...
def roidb_reader(roidb, mode):
    im, im_scales, gt_boxes, gt_classes = data_utils2.get_image_blob(roidb, mode)
    im_id = roidb['im_id']
...
def RRPNData(mode,
    ...
    def reader():
        if mode == "train":
            ...
            batch_out = []
            for roidb in roidbs:
                im, gt_boxes, gt_classes, is_crowd, im_info, im_id, is_difficult = roidb_reader(roidb, mode)
                batch_out.append((im, gt_boxes, gt_classes, is_crowd, im_info, im_id, is_difficult))
                if len(batch_out) == batch_size:
                    yield batch_out
                    batch_out = []
            if len(batch_out) != 0:
