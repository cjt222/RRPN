...
struct RBoxDecodeAndClipFunctor {
  ...
  __device__ void operator()(size_t i) {
    ...
    if (var) {
      ...
      d_g = de_g / var[k + 4] * 1.0 / 3.141592653 * 180 + angle;
    } else {
      d_cx = cx + de_cx * w;
      d_cy = cy + de_cy * h;
      d_w = exp(Min(de_w, bbox_clip_default)) * w;
      d_h = exp(Min(de_h, bbox_clip_default)) * h;
      d_g = de_g * 1.0 / 3.141592653 * 180 + angle;
    }
...
__device__ inline float inter(float const *const region1,
                              float const *const region2) {
  float pts1[8];
  float pts2[8];
  float int_pts[16];
  int num_of_inter;
...
__device__ inline float IoU(float const *const region1,
                            float const *const region2) {
