        ...
        for image in image_list:
            ...
            for gt_line in gt_split:
                ...
                if len(gt_ind) > 3 and '###' not in gt_ind[8]:
                    ...
                    if self.mode == 'val':
                        easy_boxes.append(list(np.array([pt1, pt2, pt3, pt4]).reshape(8)))
                    else:
                ...
                if len(gt_ind) > 3 and '###' in gt_ind[8]:
                    ...
                    y_ctr = float(pt1[1] + pt3[1]) / 2  # pt1[1] + np.abs(float(pt1[1] - pt3[1])) / 2
                    if self.mode == 'val':
                        hard_boxes.append(list(np.array([pt1, pt2, pt3, pt4]).reshape(8)))
                    else:
            ...
            for idx in range(len(boxes)):
                if self.mode == 'train':
                    ...
                    gt_boxes[idx, :] = [boxes[idx][0], boxes[idx][1], boxes[idx][2], boxes[idx][3], boxes[idx][4], boxes[idx][5], boxes[idx][6], boxes[idx][7]]
                gt_classes[idx] = 1
        ...
        for image in image_list:
            ...
            for gt_line in gt_split:
                ...
                if len(gt_ind) > 3:
                    ...
                    y_ctr = float(pt1[1] + pt3[1]) / 2  # pt1[1] + np.abs(float(pt1[1] - pt3[1])) / 2
                    if self.mode == 'val':
                        boxes.append(list(np.array([pt1, pt2, pt3, pt4]).reshape(8)))
                    else:
